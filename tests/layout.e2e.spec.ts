...
test.describe('Home Page Layout', () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Mock 10 sectors for horizontal scroll testing
    ...
    await page.route('**/api/v1/sectors/ranking/fluctuation?limit=10', route => route.fulfill({ status: 200, body: JSON.stringify({ data: mockSectors }) }));
  ...
  test('should render 10 AI-focused sectors and be scrollable', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByText('테스터님')).toBeVisible();
    
    // Check if 10 sector cards are rendered
    // Home.tsx uses SectorCard component which has "AI 추천" or "과열" badge
    const sectorCards = page.locator('button:has-text("섹터 ")');
    await expect(sectorCards).toHaveCount(10);
