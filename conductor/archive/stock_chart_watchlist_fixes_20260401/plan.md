...
- [x] Task: 주봉 차트 데이터 오류 분석 및 재현 테스트 작성
    - [x] `src/api/__tests__/stock.test.ts` 에서 주봉 데이터 요청 및 응답 데이터 검증 케이스 추가
    - [x] `src/app/components/screens/__tests__/StockDetail.test.tsx` (없을 경우 생성) 에서 차트 렌더링 검증 테스트 작성
- [x] Task: 주봉 데이터 정합성 오류 수정
    - [x] `src/api/stock.ts` 에서 주봉 데이터 파싱 로직 검증 및 수정
    - [x] `src/hooks/use-stock.ts` 에서 차트용 데이터 변환 로직 수정 (필요 시)
- [x] Task: 차트 렌더링 보완
